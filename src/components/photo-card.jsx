...
const PhotoCard = ({ photo }) => {
    ...
    const creationDate = photo.mediaMetadata?.creationTime
        ? new Date(photo.mediaMetadata.creationTime)
        : null;

    const formattedDate = creationDate
        ? `${String(creationDate.getFullYear()).slice(-2)}/${String(creationDate.getMonth() + 1).padStart(2, "0")}/${String(creationDate.getDate()).padStart(2, "0")}`
        : "Unknown Date";

    // Function to open the image in Google Photos
    const openInGooglePhotos = () => {
        if (photo.productUrl) {
            window.open(photo.productUrl, "_blank"); // Opens in a new tab
        } else {
            console.error("❌ No product URL available for this photo.");
        }
    };
